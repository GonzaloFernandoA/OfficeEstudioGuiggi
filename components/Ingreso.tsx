...
const Ingreso: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Aquí irá el POST cuando esté listo el BAC
      // const response = await apiClient.post('/ingreso', payload);
      // if (response.error) throw new Error(response.error);

      setMessage({ type: 'success', text: 'Ingreso guardado exitosamente (en preparación para envío).' });
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-6">
      ...
      <div className="sticky bottom-0 bg-white/80 backdrop-blur-sm py-4 -mx-8 px-8 border-t border-slate-200 z-10 flex justify-end space-x-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-6 py-3 border border-transparent shadow-lg text-base font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
